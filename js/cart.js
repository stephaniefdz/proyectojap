...
// Funcion que trae al articulo del usuario en el carrito 
  ...
  function showCart(cart) {
    ...
    for (let i = 0; i < cart.length; i++) {
        showCartHtmlToAppend += ` 
        <tr> 
            <th scope="col"><img src="${cart[i].image}" width="150px"></th>
            <th scope="col">${cart[i].name}</th>
            <th scope="col">${cart[i].currency} <span class="cost">${cart[i].unitCost}</span></th>
            <th scope="col"><input type="number" min="1" max="5" value="${cart[i].count}" style="width: 70px;"></th>
            <th scope="col">${cart[i].currency} <span class="subTotal" data-currency="${cart[i].currency}">${cart[i].unitCost * cart[i].count}</span></th>
